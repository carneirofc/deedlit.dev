...
/**
 * Modal that browses the server filesystem (via /api/library/fs/browse) and
 * returns the absolute path of a chosen folder.  Navigate into the target
 * folder, then confirm with "Select this folder".
 */
export function DirectoryPicker({
  ...
  const browse = useCallback(async (target: string | null) => {
    ...
    try {
      ...
      const res = await fetch(`/api/library/fs/browse${qs}`);
