...
import { getDetail, GatewayError, type CatalogImage } from "@/lib/api-client";
 ...
 * the gateway GET /detail/{sha256} out over the id list with bounded
 * concurrency and returns each image's RAW catalog record — the same shape the
 * API serves (contracts/catalog.openapi.yaml), NOT the lossy UI detail subset —
 * so the dump round-trips against the contract. There is no bulk detail endpoint
 * downstream, so this is just N gateway reads; per-id failures are collected in
 * `errors` so a partial export still returns whatever resolved.
 */
